...
class Tester:
    def __init__(self, config_parser):
        ...
        self.vertical_angle = ntinst.getTable("visionTable").getEntry("vertAngle")
        self.horizontal_angle = ntinst.getTable("visionTable").getEntry("horAngle")
    ...
    def label_frame(self, frame, object_name, box, score, x_scale, y_scale):
        ...
        if abs(yCenter) > 0.0:
            horAngle = math.atan(xCenter / yCenter) * 180 / pi
            vertAngle = 90 - horAngle
        elif abs(xCenter) > 0.0:
            vertAngle = math.atan(yCenter / xCenter) * 180 / pi
            horAngle = 90 - vertAngle
        self.vertical_angle.append("vertical Angle:": vertAngle)
        self.horizontal_angle.append("horiztional Angle": horAngle)
